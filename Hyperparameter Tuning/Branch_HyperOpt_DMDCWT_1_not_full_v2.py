...
Rs_path = directory + r'\Rs_FBNN_1.joblib'
Duration_path = directory + r'\Duration_FBNN_1.joblib'
...
V_feeder_CWT_1_not_full_path = directory + r'\V_feeder_DMDCWT_reduced_1_not_full.joblib'
class_path = directory + r'\Branch_Output_sorted_1.joblib'

# Change load file for different dataset
V_feeder_CWT_1 = joblib.load(V_feeder_CWT_1_not_full_path)
Branch_Output_sorted_1 = np.array(joblib.load(class_path))
...
def f_nn(params):
    dataset, output_class = V_feeder_CWT_1, Branch_Output_sorted_1
    ...
    plot_model(model, "Branch_1_ID_training_model.png", show_shapes=True)
    ...
    write_csv(df, r'\dmdcwt_branch_id_1_not_full_v2.csv')
    best_csv(df, r'\best_dmdcwt_branch_id_1_not_full_v2.csv')
...
trainX, testX, trainY, testY, trainRs, testRs, trainDuration, testDuration = shuffle_dataset(V_feeder_CWT_1,
                                                                                             Branch_Output_sorted_1,
                                                                                             Rs, Duration)
...
last_model.summary()
plot_model(last_model, "Branch_1_ID_best_model.png", show_shapes=True)
